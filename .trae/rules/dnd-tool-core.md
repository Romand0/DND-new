...
| 修改长任务流程规范（Spec / 静默模式 / 并行策略 / 错误处理） | §11 长任务降额工作流 |

> **原则**：只加不减。§7 踩坑清单采用追加式，除非重构彻底消除旧坑。临时性 hack 写代码注释加 TODO，不写进文档。

---

## 十、长任务降额流程硬约束（**完整功能 / 跨 5+ 文件任务默认启用，自动识别无需用户说明**）

> 与 AGENTS.md §11 完全一致，此处为强制精简版。用户明确说「别用长任务流程，直接写」才禁用。

### 10.1 自动触发条件（满足任一即启用）

- 用户指令开头含 **`[长任务]`** 前缀
- 任务含「搭建/实现功能/新增模块/完整功能」等关键词
- 预估涉及文件 ≥ 5 个或跨 3+ 模块（类型+store+页面+API）
- 用户明确说「走长任务流程」

### 10.2 阶段一：Spec 阶段（**只写文档，写完必须停，等用户确认才实现**）

在 `.monkeycode/specs/<feature-slug>/` 输出 3 个文件（缺一不可）：
1. **requirements.md** → 用户故事 + 输入输出表 + 边界验收标准（含 ✅正常/⚠️异常/❌排除）
2. **design.md** → 涉及文件清单（按 §一 速查表定位）+ 类型骨架预览 + 调用链路 + 权限认证 + **预读文件清单**（实现阶段一次读入）
3. **tasks.md** → 分组任务清单（标注「无依赖，可并行」的组），标准四分组：A(类型+store)/B(API)/C(UI)/D(集成收尾)

❌ 严禁 Spec 阶段写任何实现代码（src/、functions/ 下的文件）。
✅ Spec 三文件写完立即停，发通知等用户明确确认，禁止直接进入实现。

### 10.3 阶段二：实现阶段（用户确认 Spec 后，五条铁律）

| # | 规则 | 严禁 |
|---|------|------|
| ① **预读一次** | 开头一次性把 design.md §预读文件清单 全读入；store 只读 characterStore.ts 作范本，其他 8 个 store 不要全文重读 | 每做一步重新 Read 同一文件（上下文重复堆积） |
| ② **分组并行** | tasks.md 中「可并行」的组（无相互 import）用 `general_purpose_task` 并行跑，子Agent结果摘要回传 | 全部串行等前一步输出（拉长历史、堆上下文） |
| ③ **静默模式** | 每完成一大组（3-5文件）只发一行：`✅ 完成：组X <简短说明>（文件：...）`；**只在 3 种情况问用户**：Spec 冲突 / 新依赖 / 破坏性改动 | 单文件逐解释代码、加大量 educational insight、铺垫下一步（纯烧字） |
| ④ **Fail Fast** | 同一错误（type/build/逻辑）**最多改 1 次**；还不对就发：`❌ 卡壳：<错误> 已尝试：<X> 需要你确认：①A ②B ③C` | 同一问题反复改 3+ 次（每次尝试纯烧额度） |
| ⑤ **分组验证** | 每大组只跑 `npx tsc --noEmit`；**全部完成后只跑 1 次** `npm run build` | 每文件/每小组都 build 一次（vite 日志超长堆上下文） |

### 10.4 阶段三：收尾（极简摘要）

只输出一段：功能名 + 新增/修改文件列表 + tsc/build 结果 + 1 句下一步建议。
禁止附代码行解读，代码在文件里直接点链接看。
