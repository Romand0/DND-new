...
import { Grid3x3, Eraser, Trash2, ZoomIn, ZoomOut, Undo2, X, Swords, BookOpen, MoreHorizontal, Package, Pencil, PenTool } from 'lucide-react';
...
export default function Battleground({ sessionId, combatants, onRequestAttack, onRequestSpell, onPickupItem, readOnly = false, activeTurnCombatantId = null, playbackOnlyMovableId = null, combatInventories, onRemoveItem, equipmentChangesMap, onUpdateChanges, mode = 'simulation', playbackActive = false, actionsMap = {}, remainingMovementMap = {}, onConsumeMovement, onRefundMovement, onSelectionChange }: Props) {
  ...
  // 沙盘操作提示语（按模式优先级显示）
  const modeHint = playbackOnlyMovableId
    ...
    <div className="rounded-lg border dark:border-border-dark light:border-border-light dark:bg-card-dark light:bg-card-light p-3 sm:p-4 space-y-3">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {/* 绘图模式入口 */}
          <button
            onClick={() => navigate(`/combat/${sessionId}/battleground-editor`)}
            className="px-2 py-1 text-xs rounded-lg border dark:border-border-dark dark:text-text-dark light:border-border-light light:text-text-light hover:bg-primary/10 hover:text-primary transition-colors flex items-center gap-1"
            title="进入绘图模式"
          >
            <PenTool className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">绘图</span>
          </button>
        </div>
      </div>
